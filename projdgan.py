...
class PROJDGAN(object):
    def __init__(self, name, dataset_type, gan_loss_type):
        ...
        self.d_real_logits, _ = network.discriminator(self.real_images, y=self.inputs_y,
                                                      y_conditioning=True, is_training=True, use_bn=True)
        ...
        # add simple gradient penalty
        real_loss = tf.reduce_sum(self.d_real_logits)
        real_grads = tf.gradients(real_loss, [self.real_images])[0]
        r1_penalty = tf.reduce_sum(tf.square(real_grads), axis=[1, 2, 3])
        self.d_loss += tf.reduce_mean(r1_penalty * (self.r1_gamma * 0.5))
